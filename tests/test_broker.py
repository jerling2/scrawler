...
def producers(pytest_payload):
    ...
    codecs = [JsonCodec for _ in range(num_topics)]
    cbs = [on_deliver for _ in range(num_topics)]
    payloads = [pytest_payload for _ in range(num_topics)]
    producer_01 = list(zip(codecs, topics_01, payloads, keys_01, cbs))
    producer_02 = list(zip(codecs, topics_02, payloads, keys_02, cbs))
...
def consumers(pytest_payload):
    ...
    consumer_01 = IPGConsumer(topics_01, codec, on_notify)
    consumer_02 = IPGConsumer(topics_02, codec, on_notify)
